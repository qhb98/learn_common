        ...
        while True:
            ...
            for k, v_type in enumerate(model.vehicle_type_list):
                ...
                if i == j:
                    ...
                    distance[v_type] = distance[v_type] - model.distance_matrix[n_3, v_type] + model.distance_matrix[
                        n_3, n_2] \
...
def calFitness(model):
    ...
    for sol in model.sol_list:
        ...
        if ret is not None:
            sol.route_list = ret
            sol.timetable_list, sol.time_of_routes, sol.distance_of_routes, sol.obj = calTravelCost(sol.route_list,
                                                                                                    model)
            if sol.obj > max_obj:
                max_obj = sol.obj
    ...
    # calculate fitness
    for sol in model.sol_list:
        sol.fitness_cal = max_obj - sol.obj if sol.obj is not None else max_obj
    if best_sol.obj < model.best_sol.obj:
        model.best_sol = copy.deepcopy(best_sol)
    if number_of_split_failures >= model.popsize * 0.3:
        print(
            "There are {} sols that are unfeasible. Please increase the number of vehicles to obtain better solutions."
...
def selectSol(model):
    ...
    for i in range(model.n_select):
        ...
        f1_fit = sol_list[f1_index].fitness_cal
        f2_fit = sol_list[f2_index].fitness_cal
        if f1_fit < f2_fit:
...
if __name__ == '__main__':
    demand_file = r'../data/MDHVRPTW/demand.csv'
    depot_file = r'../data/MDHVRPTW/depot.csv'
    run(demand_file=demand_file, depot_file=depot_file, epochs=100, pc=0.8, pm=0.1, popsize=100, n_select=80,
        opt_type=0)
