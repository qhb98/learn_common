            ...
            for k, v_type in enumerate(model.vehicle_type_list):
                ...
                if i == j:
                    ...
                    distance[v_type] = distance[v_type] - model.distance_matrix[n_3, v_type] + model.distance_matrix[
                        n_3, n_2] + model.distance_matrix[n_2, v_type]
                route = node_id_list[i:j + 1]
                route.insert(0, v_type)
                route.append(v_type)
                if not checkTimeWindow(route, model, vehicle):
                    # 检查时间窗，只有满足时间窗才有可能生成新的标签，否则跳过
                    continue
...
def muSol(model, v1):
    ...
    mu_x = [min(int(x1[i] + model.F * (x2[i] - x3[i])), len(model.demand_id_list) - 1) for i in
            range(len(model.demand_id_list))]
    return mu_x
